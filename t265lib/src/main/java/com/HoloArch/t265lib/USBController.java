...
public class USBController {
    ...
    public int loadT265fw() {
        ...
        if (ud != null) {
            PendingIntent pi = PendingIntent.getBroadcast(this.context, 0, new Intent(UsbManager.ACTION_USB_DEVICE_ATTACHED), 0);
            IntentFilter filter = new IntentFilter(UsbManager.ACTION_USB_DEVICE_ATTACHED);
            context.registerReceiver(mUsbReceiver, filter);

            if (!usbManager.hasPermission(ud)) {
                getPremission(usbManager, ud);
                return -6;
            } else {
...
*/
    ...
    private byte[] openBin(String filename) throws IOException {
        InputStream is = context.getAssets().open("fw/target.mvcmd");
    ...
    private void getPremission(UsbManager manager, UsbDevice usbD) {
        PendingIntent pi;
        if (!manager.hasPermission(usbD)) {
